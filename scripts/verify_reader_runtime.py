...
def assessment(result, bundle, scope, reader):
    """Evaluator-only content comparison. No expected metric values enter runtime."""
    complete = result['status'] == 'COMPLETED' and len(result['steps']) == 3
    ...
    return {'run_id': result['id'], 'status': result['status'], 'calls_reserved': result['calls_reserved'],
            'native_reader_bound': bound,
            'projected_fixture_rows_match': content_matches,
            'runtime_verification_passed': complete and bound and content_matches,
...
def run(config, bundle, native_id, scope, database, key, *, execute=transport, status_only=False):
    ...
    return {'summary': assessment(result, bundle, scope, reader), 'run': result, 'bundle_hash': bundle['bundle_hash']}
