...
def outcome(state):
    ...
    classification='UNRESOLVED' if reason in ('BUDGET_LIMIT','DEADLINE','PLANNER_FAILED','TOOL_UNAVAILABLE','PLANNER_COMPLETION_UNCERTAIN','REMOTE_COMPLETION_UNCERTAIN') else 'INSUFFICIENT_EVIDENCE'
...
class AdaptiveRuntime:
    def __init__(self,runtime,planner,clock=time.time,planner_profile=None):
    ...
    def step(self,identity):
        ...
        with self.runtime.db() as db:
            ...
            if state['status'] in ('COMPLETED','NEEDS_INPUT','HELD'):return self.get(identity)
    ...
    def dispatch(self,identity,token):
        ...
        result=self.runtime.execute(child['id'])
