...
class BackgroundWorker:
    def __init__(self,store,config,estate_loader,max_jobs=1,max_seconds=900,poll_seconds=5,process=process_one,explain=None):
        ...
        try:
            while not self.stop_event.is_set() and count<self.max_jobs and time.monotonic()<deadline:
                ...
                if result['status']!='IDLE':
                    ...
                    if result['status']=='COMPLETED' and self.explain:
                        try:explanation=self.explain(result['investigation_run_id'])
                        except Exception:explanation={'status':'FAILED'}
                        self.update(last_explanation=explanation)
