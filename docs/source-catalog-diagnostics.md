...
- Execute one SELECT aggregate with MAXDOP 1, a 30-second connection timeout, 30-second command timeout and 90-second timeout per worker attempt. There are no joins, stored procedures, remote functions or writes. Only connection-stage SQL 40613 uses bounded recovery (three attempts, 10/20-second waits, up to about 300 seconds including worker limits); query-stage errors never retry. See [recovery policy](v2-diagnostic-milestone.md). These limits do not guarantee a fixed scan cost or remaining Azure quota.
