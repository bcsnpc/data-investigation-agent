...
## Bounded readback and keyed evidence

Added native/SQL projected record groups with multiplicity, strict completeness detection, keyed differences, receipt integrity, durable recovery and adaptive paired evidence. 35 focused tests include equal totals hiding changed keys and crash/cancellation paths. A live five-order comparison returned matching projected values from Power BI and SQL while preserving generation/semantic/identity proof gaps. See [scope, operational use and live evidence](keyed-readback-milestone.md). D/F/G remain in progress; H is not passed.
...
## META-008 native column-slicer context

Issue #131: PR #130 merged after all six CI checks passed. Added retained native column-slicer discovery, model-column resolution and explicit all/categorical selection handling. Missing selections request input; supplied selections never imply runtime verification. Six focused tests pass. Retained live-definition evaluation found 14 slicers across four pages, all requesting missing context and accepting explicit all choices in the evaluation. Artifact .local/slicer-context/3ac50f22-2594-46e4-b153-6a0ac16b4b7a/report.json. See [native slicer context](native-slicer-context.md). All 333 script tests passed, including six new slicer tests added to CI. No cloud, SQL business queries, LLM or delivery calls. Next: reviewed slicer context collection and native filter/measure evaluation. Remaining: runtime report verification, broader semantic defects, live snapshot/freshness guarantees, richer AI/context/lineage, real delivery acceptance, hosting/auth/recovery/monitoring and full product acceptance/demo.


## META-009 reviewed slicer intake and execution hold

Issue #133: PR #132 merged after all six CI checks passed. Tickets/API retain bounded native_slicers with native_page and idempotent scope; planning checks the selected page and persists slicer evidence. Missing choices request input, and supplied choices stay held because the bounded worker cannot apply them yet. Approval and direct worker execution refuse nonempty selections rather than silently dropping them. All 338 script tests passed, including five new reviewed-slicer tests added to CI. See [reviewed slicer context](reviewed-slicer-context.md). No cloud, SQL business queries, LLM or delivery calls. Next: supported categorical execution with comparable evidence, then UI selection controls. Remaining: native filter/DAX runtime verification, broader defects, live snapshot/freshness guarantees, richer AI/context/lineage, real delivery acceptance, hosting/auth/recovery/monitoring and full product acceptance/demo.


## LAB-011 categorical selection execution

Issue #135: PR #134 merged after all six CI checks passed. Added exact categorical predicate execution over a single-transaction local Silver/Gold capture for FactOrder order_id/currency/status. Retains full capture and selected indices, validates before filtering, and separates currencies/empty results. All 345 script tests passed, including seven new categorical replay tests added to CI. Retained native Order Operations detail definitions selected one order: baseline 99/99, injected double-refund 99/0, reset READY. Artifact .local/categorical-replay/7368a4ca-5354-49a1-9454-71aa5139c014/report.json. See [categorical replay](categorical-filter-replay.md). Production worker holds remain active; no cloud, SQL business queries, LLM or delivery calls. Next: explicitly reviewed local categorical execution, then live acquisition/filter equivalence. Remaining: slicer UI, native DAX/runtime parity, broader defects, live snapshot/freshness guarantees, richer AI/context/lineage, real delivery acceptance, hosting/auth/recovery/monitoring and full product acceptance/demo.


## LAB-012 approved one-attempt categorical replay

Issue #137: PR #136 merged after all six CI checks passed. Added a separate local prepare/show/approve/run workflow binding definitions, selections, lab path and snapshot fingerprints to a reviewed hash. Execution checks fingerprints within its capture transaction, claims once and reuses completed results; failures remain UNCERTAIN with no automatic retry. All 351 script tests passed, including six new approved replay tests added to CI. Retained native selectors detected the isolated double-refund mismatch and reset READY; artifact .local/approved-categorical/4159e0c6-ce11-466a-8711-9f12455054b6/report.json. See [approved replay](approved-categorical-replay.md). Production holds unchanged; no cloud, SQL business queries, LLM or delivery calls. Next: UI integration and live filter/snapshot parity. Remaining: slicer UI, native DAX/runtime parity, broader defects, live snapshot/freshness guarantees, richer AI/context/lineage, real delivery acceptance, hosting/auth/recovery/monitoring and full product acceptance/demo.
