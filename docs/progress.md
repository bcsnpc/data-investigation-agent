...
See [Gold snapshot publication](gold-snapshot-publication.md). Semantic refresh and metadata lineage remain follow-up work. No semantic framing was requested in this step and the legacy reporting-dimension receipt is superseded for this path; the refresh helper must consume the registered nine-table Gold receipt. End-to-end model snapshot comparability is not yet claimed.
