...
export const database = {
  ...
  async detail(id) {
    const p = await pool();
    const result = await p.request().input("id", sql.NVarChar(40), id).query(`
      SELECT o.*,c.customer_name,c.customer_segment,c.state,c.country FROM app.orders o JOIN app.customers c ON c.customer_id=o.customer_id WHERE o.order_id=@id;
    ...
    const [orders, lines, payments, shipments, refunds, audit, refundLines] =
      result.recordsets;
    return {
      order: orders[0],
      lines,
